...
function App() {
  ...
  const generateRandomString = (length) => {
    ...
    return Array.from(array, (dec) =>
      ("0" + dec.toString(16)).slice(-2).join("").substring(0, length)
    );
  };
    ...
    <div className="app">
      <header>
        <div className="header-content">
          ...
          <div className="header-center">
            <h2 className="header-title">Knowledge Management</h2>
          </div>
      ...
      <div
        className={`new-chat-container ${
          messages.length > 0 ? "new-chat-visible" : ""
        }`}
      >
  ...
  return (
    <div className="references-section">
      {references.map((ref, i) => {
        if (ref.chunkInfo.relevanceScore < 0.5) return null;
